import {createRouter,createWebHashHistory } from 'vue-router'
import Home from '@/views/Home.vue'
import Editor from '@/views/Editor/Editor.vue'
const router = createRouter({
  history: createWebHashHistory(import.meta.env.BASE_URL),
  routes:[
    {
      path:'/',
      name:'Home',
      component:Home,
      
    },
    {
      path:'/edit',
      redirect:'/edit/edit',
      children:[
        {path:'/edit/edit',name:'Edit',component: Editor},
        {path:'/edit/project',name:'Project',component: () => import('@/views/Editor/Project.vue')},
        {path:'/edit/conversion',name:'Conversion',component: () => import('@/views/Editor/Conversion.vue')},
        {path:'/edit/importpump',name:'ImportPump',component: () => import('@/views/Editor/ImportPump.vue')},
        {path:'/edit/element',name:'Element',component: () => import('@/views/Editor/Element.vue')}
      ]
    },
    {
      path: '/shop',
      // redirect: '/shop/shop',
      component: () => import('@/views/Shop/Index.vue'),
      children:[
        {path:'/shop/shop',name:'Shop',component: () => import('@/views/Shop/Shop.vue')},
        {path:'/shop/detail',name:'Detail',component: () => import('@/views/Shop/Detail.vue')},
        {path:'/shop/cart',name:'Cart',component: () => import('@/views/Shop/Cart.vue')},
      ]
    },
    {
      path: '/user',
      component: () => import('@/views/Shop/Index.vue'),
      children:[
        {path:'/user/info',name:'User',component: () => import('@/views/User/Info.vue')},
        {path:'/user/order',name:'Order',component: () => import('@/views/User/Order.vue')},
        {path:'/user/address',name:'Address',component: () => import('@/views/User/Address.vue')},
        {path:'/user/integral',name:'Integral',component: () => import('@/views/User/Integral.vue')}
      ]
    },
    {
      path:'/login',
      name:'Login',
      component: () => import('@/views/Login.vue'),
    },
    {
      path:'/register',
      name:'Register',
      component: () => import('@/views/Regist.vue'),
    },
  ]
})
router.beforeEach((to, _from, next) => {
	if (!sessionStorage.getItem('token')) { // 如果没有
		if (to.name == "Login" || to.name == "Register" || to.name == "Home") { //判断是否要去登陆界面如果是则放行
      next()
		}else { // 否则直接跳转登录界面
			router.push('/login')
		}
	} else { // 如果有则放行
    next()
  }
})
export default router